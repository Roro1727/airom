...
func addGlobalFlags(fs *pflag.FlagSet) {
	...
	fs.String("io-budget", "256m", "byte-weighted I/O semaphore budget (k/m/g suffixes)")
	fs.String("max-file-size", "1m", "full-content read cap for text detectors (k/m/g suffixes)")
	...
	fs.String("cdx-version", "1.6", "CycloneDX spec version: 1.6 or 1.7")
	fs.Bool("sarif-strict-kinds", false, `emit spec-pure kind:"informational" instead of level:"note"`)
	fs.Int("exit-code", 0, "exit status when --fail-on matches (default 1 when a policy is active)")
	...
	fs.String("pprof", "", "serve net/http/pprof on addr (bare flag: localhost:6060)")

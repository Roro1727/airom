...
| `--cve` | bool | `false` | **Opt-in CVE overlay.** Match the AI package dependencies AIROM inventoried (by their purl) against the live [OSV.dev](https://osv.dev) advisory database and attach the resulting CVEs to those components — see [cve.md](./cve.md). Scoped to AI packages, not a general-purpose SCA. Because it queries a live database it is neither offline nor deterministic across time (the same scan surfaces more CVEs as OSV grows), which is why it is off by default and **mutually exclusive with `--offline`**. Degrades honestly: a network failure yields no CVEs and a warning, never a fatal error. |
